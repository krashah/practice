...
while authenticate_git_user() =="Invalid details":
	if("Authentication Successful"==authenticate_git_user()):
	    break
...
if bool_dry:
    ...
    #release_title = title.replace(" -",'');release_text = "[ChangeLog](" + url_milestone + ")"
    release_title='testinxs1g';release_text='tessxx1ting'
    ...
    release_tag = "v" + '2122'
    try:
        re=rep.create_git_release(release_tag, release_title, release_text, draft=False, prerelease=False, target_commitish="master");
        #uri_template = URITemplate("https://uploads.github.com/repos/krashah/practice/releases/10899133/assets")
        url="https://"+init.git_username+":"+init.git_password+"@api.github.com/repos/krashah/practice/releases/latest"
        response_object= requests.get(url,auth=(init.git_username,init.git_password))
        print(response_object.status_code)
        data=json.loads(response_object.text)
        print(data);upload_url=data['upload_url']
        print(data['upload_url'])
        url = "https://uploads.github.com/repos/krashah/practice/releases/10899133/assets"
        t = URITemplate(upload_url)
        asset_url = t.expand(name='cobigen-eclipse-3.1.0.zip')
        r = requests.post(asset_url, auth=(init.git_username,init.git_password) ,headers={'Content-Type':'application/zip'}, files={'file': ('cobigen-eclipse-3.1.0.zip', open('cobigen-eclipse-3.1.0.zip', 'rb'), 'application/octet-stream')})
